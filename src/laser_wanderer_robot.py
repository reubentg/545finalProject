...
class LaserWanderer:
    ...
    """
    def __init__(self, rollouts, deltas, speed, compute_time, laser_offset, rollouts2):
        # Store the params for later
        self.rollouts = rollouts
        self.rollouts2 = rollouts2 # for possible rollouts to match the angle from line_follower.py
        self.deltas = deltas
        self.speed = speed
        ...
        self.curr_pose = None # added later only for visualizing selected pose from rollout, otherwise can use local var
        self.angle_from_line_follower = None
        self.laser_msg_example = None # to get laser angle min and max when getting index for angle from line follower
        ...
        self.float_sub = rospy.Subscriber("angle_from_line_follower", Float32, self.float_cb)
    ...
    def wander_cb(self, msg):
        global TEST_COUNT
        if self.angle_from_line_follower == None:
            return
        ...
        # float32[] ranges
        # float32[] intensities
        angle = self.angle_from_line_follower
        if angle < -0.34:
            angle = -0.34
        elif angle > 0.34:
            angle = 0.34
        # index_for_closest_angle_line_follower = int(round((angle - self.laser_msg_example.angle_min) / self.laser_msg_example.angle_increment))
        index_for_closest_angle_line_follower = int(round((angle - (-0.341)) / (0.341*2/200.)))
        print "SHAPES", self.rollouts[7,:,:].shape, self.rollouts2[7,:,:].shape, self.deltas.shape[0]
        # add variable rollout to self.rollouts
        print "TEST_COUNT", TEST_COUNT
        self.rollouts[-1,:,:] = self.rollouts2[index_for_closest_angle_line_follower,:,:]
        TEST_COUNT += 1

        # visualize selected rollout by picking any pose in the selected rollout
        pose = self.rollouts2[index_for_closest_angle_line_follower,int(T-1),:]  # self.rollouts[i, 299, :] will be an array of [x, y, theta] for the final pose (last j index) for rollout i
        translation_map_to_robot = np.array([[self.curr_pose[0]], [self.curr_pose[1]]]).reshape([2, 1])
        rotation_matrix_map_to_robot = utils.rotation_matrix(self.curr_pose[2])
        rollout_position = np.array([[pose[0]], [pose[1]]]).reshape([2, 1])
        map_position = rotation_matrix_map_to_robot * rollout_position + translation_map_to_robot
        ...
        PS.pose.position.x = map_position[0]
        PS.pose.position.y = map_position[1]
        PS.pose.position.z = 0  # set msg z position to 0 since robot is on the ground
        PS.pose.orientation = utils.angle_to_quaternion(
            pose[2] + self.curr_pose[2])  # car's yaw angle + rollout pose's angle from car
        self.line_follower_angle_pub.publish(PS)
        ...
        temp_deltas = np.append(self.deltas, np.inf) # infinity is a placeholder for the variable rollout
        delta_costs = np.zeros(temp_deltas.shape[0], dtype=np.float) # array([ 0.,  0.,  0.,  0.,  0.,  0.,  0.])
        print "Delta costs1", delta_costs
        traj_depth = 0
        ...
        while rospy.Time.now().to_sec() - start < self.compute_time and traj_depth < self.rollouts.shape[1]:
            ...
            for i in range(0, self.rollouts.shape[0]): # for each [7] rollouts
                ...
                delta_costs[i] += self.compute_cost(temp_deltas[i], self.rollouts[i, traj_depth, :], msg)
        ...
        print "Delta costs2", delta_costs
        min_delta_cost_index = np.argmin(delta_costs)
        delta = temp_deltas[min_delta_cost_index]

        # visualize selected rollout by picking any pose in the selected rollout 
        pose = self.rollouts[min_delta_cost_index, T-1, :]  # self.rollouts[i, 299, :] will be an array of [x, y, theta] for the final pose (last j index) for rollout i
        translation_map_to_robot = np.array([[self.curr_pose[0]], [self.curr_pose[1]]]).reshape([2, 1])
        rotation_matrix_map_to_robot = utils.rotation_matrix(self.curr_pose[2])
        rollout_position = np.array([[pose[0]], [pose[1]]]).reshape([2, 1])
        map_position = rotation_matrix_map_to_robot * rollout_position + translation_map_to_robot
        ...
        PS.pose.orientation = utils.angle_to_quaternion(
            pose[2] + self.curr_pose[2])  # car's yaw angle + rollout pose's angle from car
        self.selected_pub.publish(PS)
        
        #print "Selected Pose", PS
...
def generate_mpc_rollouts(speed, min_delta, max_delta, delta_incr, dt, T, car_length, add_extra_rollout_for_line_follower):

    deltas = np.arange(min_delta, max_delta, delta_incr) # array([-0.34, -0.22666667, -0.11333333,  0.,  0.11333333, 0.22666667,  0.34      ])
    if add_extra_rollout_for_line_follower:
        deltas += 0.1
        deltas = np.append(deltas, np.array([0]))
...
def main():
    ...
    rollouts, deltas = generate_mpc_rollouts(speed, min_delta, max_delta, delta_incr, dt, T, car_length,
                                             add_extra_rollout_for_line_follower=False)
    rollouts_with_extra_rollout = np.zeros((7+1,T,3))
    rollouts_with_extra_rollout[:-1, :, :] =rollouts # last rollout will be all zeros
    # rollouts is (N+1)xTx3 numpy array, where the N+ 1 is still zeros and will be used for variable angle rollout later
    # create possible variable angle rollouts
    # for checking angle from line_follower vs the other rollouts and seeing which rollout is best
    rollouts2, deltas2 = generate_mpc_rollouts(speed, min_delta, max_delta, np.abs(min_delta)/100, dt, T, car_length,
                                               add_extra_rollout_for_line_follower=False)

    #print "Constructing Laser Wanderer"
    # Create the LaserWanderer
    lw = LaserWanderer(rollouts_with_extra_rollout, deltas, speed, compute_time, laser_offset, rollouts2)
